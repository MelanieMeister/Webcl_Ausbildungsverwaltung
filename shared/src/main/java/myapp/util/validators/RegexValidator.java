...
public class RegexValidator extends RootValidator<String> {
    ...
    public static RegexValidator forAnything(String errorMessage) {
        return new RegexValidator("[\\S\\s]*", errorMessage);
    }

    public static RegexValidator forPositiveInt(String errorMessage) {
        return new RegexValidator("^[0-9]{1,10}$", errorMessage);
    }

    public static RegexValidator forPositiveLong(String errorMessage){
        return new RegexValidator("^[0-9]{1,19}$", errorMessage);
    }

    public static RegexValidator forFormattedLong(String errorMessage){
        return new RegexValidator("^[+-]?[0-9']{1,25}$", errorMessage);
    }

    public static RegexValidator forFormattedInt(String errorMessage){
        return new RegexValidator("^[+-]?[0-9']{1,14}$", errorMessage);
    }

    public static RegexValidator forFormattedFloatingPointNumber(String errorMessage){
        return new RegexValidator("^[-+]?[0-9']*\\.?[0-9]+([eE][-+]?[0-9]+)?$", errorMessage);
    }

    public static RegexValidator forBoolean(String errorMessage){
        return new RegexValidator("^((?i)true){1}|((?i)false){1}$", errorMessage);
    }
